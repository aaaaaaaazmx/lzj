...
import static com.example.lcsrq.R.id.ll_state;
import static com.example.lcsrq.R.id.pullToRefreshView;
    ...
    Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.arg1 == 1){
                ...
                adapter = new JuBaoAdapter(MyJuBaoActivity.this);
                adapter.setList(jbData);
                lv_jubao.setAdapter(adapter);
//                adapter = new JuBaoAdapter(MyJuBaoActivity.this);
//                adapter.setList(hcData);
//                lv_jubao.setAdapter(adapter);
            }
            else if (msg.arg2 ==2){
                // 地区联级
...
//                    options1Items.add("不限");
                ...
                // 获取供应站id
                //标题文字颜色
                optionsPopupWindow = new OptionsPickerView.Builder(MyJuBaoActivity.this, new OptionsPickerView.OnOptionsSelectListener() {
                    @Override
...
//        // 如果区县ID是空的
        ...
        hcRepData.setType(2);
        loginModel.getListOfHdhc(MyJuBaoActivity.this, hcRepData, new OnLoadComBackListener() {
            @Override
            public void onSuccess(Object msg) {
                type_page_progress.showContent();
                jbData = (ArrayList<JuBaoBean>) msg;
                closeDialog();
...
//        // 如果区县ID是空的
        if (!TextUtils.isEmpty(qXid)) {
            hdRepData.setAreaid(Integer.parseInt(qXid));
        }
        // 筛选状态
        //!TextUtils.isEmpty(tv_state.getText().toString())
        if (!TextUtils.isEmpty(tv_state.getText().toString())){
        ...
        hdRepData.setStatus(state); //筛选条件
        hdRepData.setType(1);
        loginModel.getListOfHdhc(MyJuBaoActivity.this, hdRepData, new OnLoadComBackListener() {
            @Override
            public void onSuccess(Object msg) {
                type_page_progress.showContent();
...
//        // 如果区县ID是空的
        ...
        hcRepData.setStatus(state); //筛选条件
        // 设置页数
        hcRepData.setPage(page);
        hcRepData.setType(2);
        loginModel.getListOfHdhcforMY(MyJuBaoActivity.this, hcRepData, new OnLoadComBackListener() {
            @Override
            public void onSuccess(Object msg) {
                type_page_progress.showContent();
                loadMoreData = (ArrayList<HdhcRespData>) msg;
                closeDialog();
                pullToRefreshView.onHeaderRefreshComplete();
                pullToRefreshView.onFooterRefreshComplete();
            ...
            @Override
            public void onError(String msg) {
                Toast.makeText(MyJuBaoActivity.this, "没有更多的数据了!", Toast.LENGTH_LONG).show();
                closeDialog();
                pullToRefreshView.onHeaderRefreshComplete();
                pullToRefreshView.onFooterRefreshComplete();
            }
    ...
    protected void addAction() {
        ...
        lv_jubao.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override

            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (flag == 1){
                    // 跳传黑点详情页面
                    ArrayList<AllCclistRespData> cclist = jbData.get(position).getCclist();
                    Intent intent = new Intent(MyJuBaoActivity.this, DfzwDetaiActivity.class);
                    intent.putExtra("dfzw", (Serializable) cclist);
                    intent.putExtra("ID", jbData.get(position).getId());
//                    intent.putExtra("state",jbData.get(position).getStatus());
...
//                    intent.putExtra("state",hcData.get(position).getStatus());
    ...
    private void getDiQu() {
        ...
        loginModel.getListOfGyzRegion(MyJuBaoActivity.this, contentGyzRegionReqData, new OnLoadComBackListener() {
            @Override
            public void onSuccess(Object msg) {
    ...
    protected void findViews() {
        ...
        lv_jubao = (ListView) findViewById(R.id.lv_jubao);
...
//            // 清楚状态
        ...
        if (v.getId() == R.id.commonLeftBtn) {
            ...
            if (TextUtils.isEmpty(tv_quxian.getText().toString())) {
                Toast.makeText(MyJuBaoActivity.this, "请您先选择区县", Toast.LENGTH_SHORT).show();
            }
            // 表示选择的是不限制
            ...
            //返回的分别是三个级别的选中位置
            //标题文字颜色
            optionsPopupWindowJD = new OptionsPickerView.Builder(MyJuBaoActivity.this, new OptionsPickerView.OnOptionsSelectListener() {
                @Override
                public void onOptionsSelect(int options1, int option2, int options3, View v) {
                    //返回的分别是三个级别的选中位置
                    //返回的分别是三个级别的选中位置
            ...
            optionsPopupWindowState = new  OptionsPickerView.Builder(MyJuBaoActivity.this, new OptionsPickerView.OnOptionsSelectListener() {
                @Override
                public void onOptionsSelect(int options1, int option2, int options3 ,View v) {
